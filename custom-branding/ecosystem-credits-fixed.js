(function() {
    ...
    console.log('🌐 Affinify: Carregando sistema FIXO de ecossistema e créditos...');

    // Função para criar os elementos do ecossistema e créditos
    function createEcosystemAndCredits() {
        // Remover elementos existentes
        document.querySelectorAll('[id*="ecosystem"], [id*="credit"], [id*="affinify"]').forEach(el => el.remove());
        
        // Procurar pelo Temporary Chat ou elementos na área superior direita
        const allElements = Array.from(document.querySelectorAll('*'));
        let targetElement = null;
        
        // Procurar por elementos que contenham "Temporary" ou "Chat"
        ...
        // Se não encontrar "Temporary Chat", procurar na área superior direita
        if (!targetElement) {
            const topRightElements = allElements.filter(el => {
                const rect = el.getBoundingClientRect();
                return rect.top < 100 && rect.right > window.innerWidth - 200;
            });
            
            targetElement = topRightElements.find(el => 
    ...
    function createElementsAtPosition(rect) {
        ...
        ecosystemIcon.id = 'affinify-ecosystem-fixed';
        ...
        creditsIcon.id = 'affinify-credits-fixed';
        creditsIcon.style.cssText = `
            position: fixed;
            top: ${rect.top}px;
            left: ${rect.left - 50 - 10 - 100}px;
            display: flex;
            align-items: center;
            gap: 6px;
            padding: 8px 12px;
        ...
        console.log('✅ Affinify: Elementos criados!');
    }
    
    // Função do popup do ecossistema
    function createEcosystemPopup() {
        const existingPopup = document.getElementById('affinify-ecosystem-popup-fixed');
        if (existingPopup) {
            existingPopup.remove();
        }
        
        const popup = document.createElement('div');
        popup.id = 'affinify-ecosystem-popup-fixed';
        popup.style.cssText = `
            position: fixed;
            top: 0;
            left: 0;
        ...
        content.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 32px;">
                ...
                <button id="close-popup-affinify-fixed" style="background: transparent; border: none; color: #9ca3af; cursor: pointer; padding: 8px; border-radius: 8px;">
        ...
        // Event listeners
        document.getElementById('close-popup-affinify-fixed').addEventListener('click', () => {
            popup.remove();
        });
    ...
    // Re-executar a cada 3 segundos para garantir que os elementos permaneçam
    setInterval(createEcosystemAndCredits, 3000);
    
    console.log('✅ Affinify: Sistema FIXO de ecossistema e créditos carregado!');
})();
